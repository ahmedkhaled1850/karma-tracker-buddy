...
export const BreakScheduler = ({ performanceId }: BreakSchedulerProps) => {
  ...
  const shiftStartDate = useMemo(() => {
    if (!shiftStart || isOffDay) return null;
    ...
    // Check if we are currently in a shift that started yesterday
    ...
    const loadSettings = async () => {
      ...
      try {
        ...
        if (dailyShift) {
          ...
          setIsOffDay(!!staticShift?.is_off_day);
  ...
  useEffect(() => {
    ...
    if (stored) {
      try {
        ...
        if (parsed.key && parsed.endTime) {
          const now = getEgyptTime().getTime();
          ...
          if (remaining > 0) {
            ...
            setEndTimeTimestamp(parsed.endTime);
  ...
  // Timer effect
  useEffect(() => {
    if (activeBreak && endTimeTimestamp) {
      startCountdown();
    }
    return () => stopCountdown();
  }, [activeBreak, endTimeTimestamp]);

  // Handle visibility change to refresh timer immediately
  useEffect(() => {
    const handleVisibility = () => {
      if (document.visibilityState === 'visible' && endTimeTimestamp) {
        const now = getEgyptTime().getTime();
        setTimeLeft(Math.max(0, Math.floor((endTimeTimestamp - now) / 1000)));
      }
    };
  ...
  useEffect(() => {
    if (activeBreak || isOffDay) {
      if (isOffDay) setNextCountdown("");
      return;
    }
    const id = window.setInterval(() => {
      ...
      if (shiftStartDate && shiftEndDate) {
        ...
        if (now < startMs) {
          setNextCountdown(formatDiff(startMs - now));
          return;
        }
        if (now <= endMs) {
          if (nextUp) {
            setNextCountdown(formatDiff(nextUp.start.getTime() - now));
            return;
          }
          setNextCountdown(formatDiff(endMs - now));
          return;
        }
        const tomorrowStart = startMs + 24 * 3600 * 1000;
        setNextCountdown(formatDiff(tomorrowStart - now));
        return;
      }
      if (nextUp) {
        setNextCountdown(formatDiff(nextUp.start.getTime() - now));
  ...
  }, [activeBreak, nextUp, shiftStartDate, shiftEndDate, isOffDay]);

  // Broadcast next event info to header
  useEffect(() => {
    const getNextEventLabel = () => {
      if (isOffDay) return "Scheduled OFF";
    ...
    const countdown = isOffDay ? "Enjoy Day Off" : activeBreak ? formatTimeLeft(timeLeft) : nextCountdown;
  ...
  }, [nextCountdown, activeBreak, timeLeft, nextUp, shiftStartDate, shiftEndDate, isOffDay]);

  // Schedule auto-start for breaks
  useEffect(() => {
    autoStartTimeoutsRef.current.forEach((id) => window.clearTimeout(id));
    autoStartTimeoutsRef.current = [];
    if (!shiftStartDate || !shiftEndDate || isOffDay) return;
    const now = getEgyptTime().getTime();
    (["break1", "break2", "break3"] as BreakKey[]).forEach((key) => {
      ...
      if (delay > 0 && breakTime.getTime() <= shiftEndDate.getTime()) {
        const id = window.setTimeout(() => {
          if (!activeBreak) {
            const start = getEgyptTime().getTime();
            const end = start + DURATIONS[key] * 1000;
            setActiveBreak(key);
            setTimeLeft(DURATIONS[key]);
            setCurrentStart(start);
            setEndTimeTimestamp(end);
            setCurrentBreakKey(key);
            localStorage.setItem("ktb_active_break", JSON.stringify({ key, start, endTime: end }));
  ...
  }, [schedule, shiftStartDate, shiftEndDate, activeBreak, isOffDay]);
  ...
  const scheduleNotifications = () => {
    ...
    (["break1", "break2", "break3"] as BreakKey[]).forEach((key) => {
      ...
      // Notify 1 min before break starts
      const beforeStartMs = breakTime.getTime() - now - 60 * 1000;
      if (beforeStartMs > 0) {
        const id = window.setTimeout(() => {
          notify("Break Reminder", `${BREAK_LABELS[key]} starts in 1 minute`);
        }, beforeStartMs);
        timeoutsRef.current.push(id);
      }

      // Notify 1 min before end
      const end = new Date(breakTime.getTime() + DURATIONS[key] * 1000);
      const beforeEndMs = end.getTime() - now - 60 * 1000;
      if (beforeEndMs > 0) {
        const id = window.setTimeout(() => {
          notify("Break Ending Soon", `1 minute left in ${BREAK_LABELS[key]}`);
        }, beforeEndMs);
        timeoutsRef.current.push(id);
      }
    });
  ...
  const scheduleShiftNotifications = () => {
    ...
    if (!shiftStartDate || !shiftEndDate || isOffDay) return;
    const now = getEgyptTime().getTime();
    const beforeStartMs = shiftStartDate.getTime() - now - 5 * 60 * 1000;
    const beforeEndMs = shiftEndDate.getTime() - now - 5 * 60 * 1000;
    if (beforeStartMs > 0) {
      const id = window.setTimeout(() => {
  ...
  const startCountdown = () => {
    ...
    intervalRef.current = window.setInterval(() => {
      if (!endTimeTimestamp) return;
      const now = getEgyptTime().getTime();
      const remain = Math.max(0, Math.floor((endTimeTimestamp - now) / 1000));
      ...
      if (remain <= 0) {
        ...
        if (currentStart && currentBreakKey) {
          const end = getEgyptTime().getTime();
    ...
    <div className="space-y-4">
      ...
      <Card className="p-4 border border-border/60">
        ...
        <div className="relative">
          ...
          {/* Next event hint */}
          {shiftStartDate && shiftEndDate && !activeBreak && !isOffDay && (
            <div className="mb-4 px-3 py-2 rounded-lg bg-muted/50 text-center">
              <span className="text-xs text-muted-foreground">
                {getEgyptTime().getTime() < shiftStartDate.getTime() ? `Shift starts at ${formatClock(shiftStartDate)}` :
                 getEgyptTime().getTime() <= shiftEndDate.getTime() ? (
          ...
          {isOffDay && (
             <div className="mb-4 px-3 py-6 rounded-2xl bg-destructive/5 border border-dashed border-destructive/20 text-center space-y-2">
               <p className="text-2xl">🏝️</p>
               <p className="text-sm font-bold text-destructive/80 uppercase tracking-widest">Enjoy Your Off Day</p>
               <p className="text-[10px] text-muted-foreground">No shifts or breaks scheduled for today</p>
             </div>
          )}
